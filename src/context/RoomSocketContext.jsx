...
/**
 * Owns ONE STOMP connection for the lifetime of a room, shared across every
 * page inside it. Because the connection lives above the routed pages,
 * navigating between them doesn't tear down and rebuild the socket — so
 * broadcasts are never dropped in a reconnect window. Outbound sends are queued
 * until the socket is connected.
 *
 * Room-agnostic: callers supply which topics to subscribe to and which
 * destination to publish a resync to. The duel flow and the Listening Lounge
 * both use this rather than maintaining two copies of the connection,
 * token-refresh and outbox logic — that behaviour was hard-won (see the
 * onStompError comment) and is exactly the kind of thing that shouldn't be
 * forked.
 *
 * @param topics           STOMP destinations to subscribe to on every connect
 * @param syncDestination  published on every (re)connect so the server can
 *                         re-send current state; omit to skip
 * @param syncBody         body for that publish
 */
export function RoomSocketProvider({ topics, syncDestination, syncBody, children }) {
  ...
  // Latest sync target without making it an effect dependency -- a fresh
  // syncBody object each render must not rebuild the socket. Updated in an
  // effect rather than during render, same as useRoomEvents' handlerRef below.
  const syncRef = useRef({ syncDestination, syncBody })
  useEffect(() => {
    syncRef.current = { syncDestination, syncBody }
  })
  ...
  // Depend on the joined string, not the array identity: callers building a
  // fresh array each render would otherwise reconnect on every render.
    ...
    const client = new Client({
      ...
      onConnect: () => {
        ...
        topicList.forEach((topic) =>
          client.subscribe(topic, (msg) => dispatchEvent(JSON.parse(msg.body)))
        )
        setIsConnected(true)
        // Ask the server where we are on every (re)connect so a reconnect or a
        // full refresh self-heals to the correct state instead of getting stuck.
        const { syncDestination: dest, syncBody: body } = syncRef.current
        if (dest) client.publish({ destination: dest, body: JSON.stringify(body ?? {}) })
        ...
        outboxRef.current = []
        queued.forEach(({ destination, body: queuedBody }) =>
          client.publish({ destination, body: JSON.stringify(queuedBody) })
        )
      },
  ...
  return (
    <RoomSocketContext.Provider value={{ send, subscribe, isConnected }}>
      {children}
...
/**
 * Register a handler for room events. The handler ref is kept current without
 * re-subscribing on every render, and `subscribe` is stable, so this attaches
 * exactly one listener for the component's lifetime.
 */
// eslint-disable-next-line react-refresh/only-export-components
export function useRoomEvents(handler) {
  const ctx = useContext(RoomSocketContext)
  if (!ctx) throw new Error('useRoomEvents must be used within RoomSocketProvider')
